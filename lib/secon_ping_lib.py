...
def quiet_ping(dest_addr, timeout = 2, count = 4, psize = 64):
	...
	return percent_lost, mrtt, artt, message


def check(dest_addr, timeout = 1, count = 1, psize = 64):
	...
	for i in xrange(count):
		...
		if delay == None:
			message = 'timeout within %ssec.' % timeout
		else:
			delay = delay * 1000
			plist.append(delay)
	...
	if 0 == artt:
		log.error('secon_ping_lib.check', '[%s] %s' % (dest_addr, message))
